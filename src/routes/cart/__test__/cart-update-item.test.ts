...
it('update items to cart', async () => {
  ...
  const res5 = await request(app)
    .patch(CartPath.withBase(CartPath.MY + CartPath.Item))
    .set('authorization', `Bearer ${token}`)
    .send({
      id: res4.body.data.data.items[1].id,
      quantity: 7,
    })
    .expect(200);

  console.log(res5.body);
  expect(res5.body.data.data.items[0].quantity).toEqual(3);
  expect(res5.body.data.data.items[1].quantity).toEqual(7);
});
