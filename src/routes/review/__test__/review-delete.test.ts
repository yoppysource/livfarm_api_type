...
it('user can delete review', async () => {
  ...
  await request(app)
    .delete(ReviewPath.withBase(ReviewPath.ID).replace(':id', res2.body.data.data[0].id))
    .set('authorization', `Bearer ${userToken2}`)
    .expect(204);
  ...
  expect(res4.body.data.data.ratingsAverage).toEqual(4);
});
